...
async def download_template_vente():
    ...
    green_fill   = PatternFill("solid", fgColor="8B1A1A")
    section_fill = PatternFill("solid", fgColor="FFF0F0")
...
async def import_catalogue_vente(fichier: UploadFile = File(...), _=Depends(require_admin)):
    ...
    # Ligne 1 = en-têtes, ligne 2 = notes, ligne 3 = exemple → données à partir de la ligne 4
    async with get_db() as db:
        for row_num in range(4, ws.max_row + 1):
            nom = col(row_num, "nom")
            if not nom:
                continue
            if nom.strip().lower() in EXEMPLE_NOMS:
                continue
