...
def get_mqtt_status() -> dict:
    """Retourne l'état de connexion MQTT et les dernières mesures reçues."""
    return {
        "connecte": _mqtt_connected,
        "broker": f"{MQTT_BROKER}:{MQTT_PORT}",
        "topic": MQTT_TOPIC,
        "nb_sondes_live": len(_live_data),
        "sondes": {
            name: {
                "temperature": d.get("temperature"),
                "humidity":    d.get("humidity"),
                "battery":     d.get("battery"),
                "linkquality": d.get("linkquality"),
                "timestamp":   d.get("timestamp"),
            }
            for name, d in _live_data.items()
        },
    }
...
def _on_connect(client, userdata, connect_flags, reason_code, properties):
    ...
    if not reason_code.is_failure:
        ...
        logger.info("✅ MQTT connecté — abonné à %s (broker %s:%d)",
                    MQTT_TOPIC, MQTT_BROKER, MQTT_PORT)
    else:
        _mqtt_connected = False
        logger.error("❌ Échec connexion MQTT : %s (broker %s:%d)",
                     reason_code, MQTT_BROKER, MQTT_PORT)
...
def _on_disconnect(client, userdata, disconnect_flags, reason_code, properties):
    ...
    if reason_code.value != 0:
        logger.warning("⚠️  MQTT déconnecté (%s) — reconnexion automatique", reason_code)
...
def _run_mqtt_thread():
    ...
    Utilise paho-mqtt CallbackAPIVersion.VERSION2 (paho ≥ 2.x).
    """
    ...
    try:
        ...
        logger.error("❌ Impossible de se connecter à %s:%d — %s", MQTT_BROKER, MQTT_PORT, e)
        logger.error("   → Vérifier MQTT_BROKER dans .env (actuellement '%s')", MQTT_BROKER)
        return
